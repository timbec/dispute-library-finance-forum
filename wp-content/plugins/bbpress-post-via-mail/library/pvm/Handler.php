...
interface pvm_Handler {
	 ...
	 * @param pvm_Message $message Message to send
	 ...
	 *
	 * pvm can operate in one of two modes:
	 *
	 * 1. Leader Mode - pvm will set the message ID for each message, and
	 *    assume full control over them for threading purposes.
	 * 2. Follower Mode - pvm will observe message IDs set by the handler,
	 ...
	 *
	 * e.g. `add_settings_field('bb_pvm_postmark_apikey', 'Postmark API Key', array(__CLASS__, 'field_apikey'), $group, $section)`
	 *
	 * Note that you need to use "bb_pvm_handler_options" as the name of the POST
	 * variable in your callback.
	 *
	 * e.g. `echo '<input type="text" name="bb_pvm_handler_options[api_key]" />`;
	 *
	 ...
	 * bb_pvm_handler_options and is expected to return a sanitized version. The
