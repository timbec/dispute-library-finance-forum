 ...
 * @subpackage Handlers
 */
class pvm_Handler_Mandrill implements pvm_Handler {
	/**
	...
	public function __construct($options) {
		if (empty($options) || empty($options['api_key'])) {
			throw new Exception(__('Mandrill API key not set', 'pvm'));
		}
	...
	protected function send_single($data) {
		...
		switch ($code) {
			...
			case 500:
				throw new Exception(__('Mandrill server error', 'pvm'), 500);
			default:
				throw new Exception(__('Unknown error', 'pvm'), (int) $code);
		}

		$data = json_decode( wp_remote_retrieve_body( $response ) );
		if ( empty( $data ) ) {
			throw new Exception(__('Invalid response from Mandrill', 'pvm'));
		}
	...
	public function handle_post() {
		if ( isset( $_POST['mandrill_events'] ) ) {
			...
			list($reply->post, $reply->site, $reply->user, $reply->nonce) = pvm_Reply::parse_to( $parsed->msg->email );
...
?>
		<p><?php
		printf(
			__("Once you've set up your API key here, add an Inbound route in your <a href='%s'>Mandrill inbound dashboard</a> to <code>%s</code>", 'pvm'),
...
<?php
	...
	public static function register_option_fields($group, $section, $options) {
		...
		add_settings_field('bb_pvm_mandrill_apikey', __('Mandrill API Key', 'pvm'), array(__CLASS__, 'field_apikey'), $group, $section);
	}
...
?>
		<input type="text" name="bb_pvm_handler_options[api_key]"
			id="bb_pvm_mandrill_apikey" value="<?php echo esc_attr($key) ?>"
			class="regular-text code" />
		<p class="description">
			<?php printf(
				__("You'll need to create an API key on the Mandrill site. Head to <a href='%s'>your settings</a>, then generate an API key under the credentials tab", 'pvm'),
