...
require_once PVM_PATH . '/vendor/postmark-inbound/lib/Postmark/Autoloader.php';
 ...
 * @subpackage Handlers
 */
class pvm_Handler_Postmark implements pvm_Handler {
	/**
	...
	public function __construct($options) {
		if (empty($options) || empty($options['api_key'])) {
			throw new Exception(__('Postmark API key not set', 'pvm'));
		}
			...
			if ( ! empty( $options['references'] ) ) {
				...
				$data['Headers'][] = array(
					...
					'Value' => $references,
                        ...
                        //error_log ("Postmark data:".$debug_export);
			$messages[ $user->ID ] = $this->send_single($data);
		}
                $debug_export = var_export($messages, true);
      	       	//error_log ("Postmark retval:".$debug_export);
		return $messages;
	}
	...
	protected function send_single($data) {
		...
		switch ($code) {
			...
			case 401:
				throw new Exception(__('Invalid API key', 'pvm'), 401);
			case 422:
				throw new Exception(sprintf(__('Error with sent body: %s', 'pvm'), wp_remote_retrieve_body($response)), 422);
			case 500:
				throw new Exception(__('Postmark server error', 'pvm'), 500);
			default:
				throw new Exception(__('Unknown error', 'pvm'), $code);
		}

		$data = json_decode( wp_remote_retrieve_body( $response ) );
		if ( empty( $data ) ) {
			throw new Exception(__('Invalid response from Postmark', 'pvm'));
		}
	...
	public function handle_post() {
		...
		$reply->attachments = $inbound->Attachments();
		list($reply->post, $reply->site, $reply->user, $reply->nonce) = pvm_Reply::parse_to($to[0]->Email);
	 ...
	 *
	 * Postmark lets us set our own message ID, so allow pvm to operate in
	 * Leader Mode.
	 *
	 * @return bool
	 */
...
?>
	<p><?php printf(
		__("Once you've set up your API key here, make sure to set your Inbound Hook URL to <code>%s</code>", 'pvm'),
...
<?php
	...
	public static function register_option_fields($group, $section, $options) {
		...
		add_settings_field('bb_pvm_postmark_apikey', __('Postmark API Key', 'pvm'), array(__CLASS__, 'field_apikey'), $group, $section);
	}
...
?>
		<input type="text" name="bb_pvm_handler_options[api_key]"
			id="bb_pvm_postmark_apikey" value="<?php echo esc_attr($key) ?>"
			class="regular-text code" />
		<p class="description">
			<?php _e("You'll need to create an API key on the Postmark site. Head to <a href='https://postmarkapp.com/servers'>your server</a>, then generate an API key under the credentials tab", 'pvm') ?>
		</p>
<?php
	}
